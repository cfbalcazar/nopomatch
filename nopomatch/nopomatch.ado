/*
*! v.3 		09/09/2013		Estimates gap for binary variables	By: Felipe Balcazar
*! v.2 		04/18/2013		Estimation of the Standard Error of Delta 0 much more efficient		By: Alejandro Hoyos
*! v.1 		05/24/2010 		Non-parametric matching decomposition By: Juan Pablo Atal, Alejandro Hoyos, and Hugo Nopo
*/
...
*======================================================================
* Deltas - computes the components of the gap

capture program drop deltas
program define deltas
version 10.1
	{
	syntax varlist [if], match(string) outcome(string) supp(string) by(string) [fact(string)] [filename(string)] [binary]
...
version 10.1
{
	syntax varlist(min=1) [if], outcome(string) by(string) [sd] [replace] [reportby(string)] [fact(string)] [filename(string)] [binary]	
	if "`replace'"!=""{
		cap drop _match
		cap drop _supp
	}
